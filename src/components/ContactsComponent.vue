  ...
  <div class="contacts-container">
    ...
    <div class="contacts-list">
      ...
      <div v-if="whatsappContact" class="contact-item">
        ...
        <a :href="whatsappUrl" target="_blank" class="contact-link">
          Whatsapp
        </a>
      </div>
      ...
      <div v-if="contactmaxContact" class="contact-item">
        <div class="contact-icon">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M12 4C7.58 4 4 7.13 4 11c0 1.72.71 3.29 1.89 4.51l-.71 3.09c-.14.62.5 1.12 1.06.82l3.03-1.62c.85.13 1.77.2 2.73.2 4.42 0 8-3.13 8-7s-3.58-7-8-7Zm0 2.55c3 0 5.45 1.98 5.45 4.45S15 15.45 12 15.45c-.9 0-1.72-.13-2.45-.4a1.27 1.27 0 0 0-1.05.08l-.19.1.08-.35c.1-.45-.05-.91-.39-1.22A3.58 3.58 0 0 1 6.55 11C6.55 8.53 9 6.55 12 6.55Zm-2.15 4.52c0-.98.86-1.82 2.15-1.82s2.15.84 2.15 1.82-.86 1.82-2.15 1.82-2.15-.84-2.15-1.82Z" fill="currentColor"/>
          </svg>
        </div>
      ...
      <div v-if="emailContact" class="contact-item">
        ...
        <a :href="emailUrl" class="contact-link">
          {{ emailContact }}
        </a>
      </div>
    </div>
...
export default {
  ...
  computed: {
    whatsappUrl() {
      ...
      const message = this.$t('contactMessage', { domainName: this.domainName });
      const encodedMessage = encodeURIComponent(message);
      return `https://wa.me/${this.whatsappContact.replace(/[^0-9]/g, '')}?text=${encodedMessage}`;
    },
    emailUrl() {
      if (!this.emailContact) return '';
      const subject = this.$t('contactMessage', { domainName: this.domainName });
      const encodedSubject = encodeURIComponent(subject);
      return `mailto:${this.emailContact}?subject=${encodedSubject}`;
    }
  },
  data() {
    return {
